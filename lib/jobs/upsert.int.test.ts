...
test('upsertJob inserts, then updates the same job on duplicate source+external_id', async (ctx) => {
...
test('upsertJob inserts a new row (updated: false) when no external_id is given', async (ctx) => {
  await tolerateOutage(ctx, async () => {

...
**ค้าง — `parseLinkedInCsv` ฝัง `source: 'scraper'` ไว้ตายตัว** แถวที่คนพิมพ์เองผ่าน
เทมเพลตนี้จึงถูกบันทึกว่ามาจาก scraper ซึ่ง**ไม่จริง และไปโผล่ผิดในกราฟ
"ผู้สมัครตามแหล่งที่มา" ที่มีไว้เป็นหลักฐาน PDPA พอดี** แก้ได้ด้วยพารามิเตอร์
`source` ที่ default เป็น `'scraper'` (สคริปต์รายคืนไม่กระทบ) แล้วให้
`/api/ingest` type `linkedin` ส่ง `'csv'` — ยังไม่ทำ เพราะเปลี่ยนความหมายของ
แถวที่มีอยู่แล้วและควรตัดสินใจแยกจากงานนี้
